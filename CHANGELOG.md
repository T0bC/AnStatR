...
- **Prediction distance comparison (PLS-DA/sPLS-DA)**: The Component Diagnostics panel now reports cross-validated Overall Error and BER under all three mixOmics prediction distances — maximum, centroid and Mahalanobis — side by side, followed by a plain-language reading of how far they disagree (under 2 pp: the choice does not matter; 2–5 pp: worth noting; over 5 pp: the groups are not cleanly separated and the headline accuracy is one of several defensible numbers). `mixOmics::perf()` already computed all three and the app discarded two of them, so this costs no extra computation. Reported accuracy, the confusion matrix, posterior probabilities and exported `.rds` bundles all stay on maximum distance, which is what `mixOmics::predict()` applies — the table is evidence, not a setting, and deliberately so: choosing the rule that gives the lowest error after seeing the table would bias the reported figure. The comparison is also written to a new Excel sheet
- **keepX Tuning Evidence panel (sPCA)**: `mixOmics::tune.spca()`'s `$cor.comp` output — the correlation between the cross-validated and full-data component at each candidate keepX, which is the actual criterion behind the chosen value — was previously discarded. It is now plotted per component with the selection marked, alongside a pasteable statement of the folds, repeats and grid used. A curve that plateaus before the selected value means a shorter variable list is available at no cost to stability; a low or erratic curve means the selection is not reproducible at this sample size and the variable list should be reported as provisional
- **Cumulative-variance thresholds (PCA/sPCA)**: The Optimal Number of Components panel now lists the components needed to reach 90% and 95% of total variance alongside Kaiser, Elbow and Parallel Analysis, wiring up `extract_variance_explained()` which was implemented and exported but had no caller. Documented as reporting conventions rather than statistical tests, since neither compares against what random data would produce. Suppressed for IPCA, whose components carry no variance ordering
- **Error Rate per Group (PLS-DA/sPLS-DA)**: The Component Diagnostics panel now breaks the cross-validated error down per group under each distance rule, colour-coded, with a note naming the worst-performing group. `mixOmics::perf()` already returned this and the app discarded it. An acceptable overall BER can conceal one group being classified at or below chance while the others carry the average — the aggregate figures cannot show this. Distinct from the existing Confusion Matrix per-class metrics, which are resubstitution-based and therefore optimistic; these are cross-validated. Also exported to a new Excel sheet
- **Suggested Component Count (mixOmics)**: The panel now also reports mixOmics' own component-count recommendation from `perf()`'s `$choice.ncomp` — one-sided t-tests between successive components, per error measure and per distance rule — as a second opinion alongside the existing parsimony heuristic, flagging whether the two agree. Neither is applied automatically
- **Cross-validation settings guidance**: Both modules now warn when folds or repeats fall below mixOmics' published advice (at least 5–6 samples per fold; 50–100 repeats for a final reported result). The app's defaults are set for responsiveness and give a provisional answer, which was previously not stated anywhere — a user could reasonably have quoted a 3-repeat figure in a manuscript. The warning distinguishes a borderline fold count from one that will fail outright (under 3 samples per fold)
- **Up-front runtime estimates**: Each opt-in cross-validation button (sPCA keepX tuning, sPLS-DA keepX tuning, component-count check) now shows an estimated runtime that updates live with the folds, repeats and grid settings, escalating to a warning banner for jobs over two minutes. Deliberately coarse ("a few seconds", "roughly 2–3 minutes") — an honestly vague estimate is more useful than a precise-looking wrong one. New shared `app/view/shared/tuning_controls.R`
- **sPCA tuning settings**: Folds, repeats and the keepX candidate grid are now user-configurable in a collapsed "Tuning settings" accordion, matching the LDA module's existing advanced panel. Previously hardcoded to 5 folds × 3 repeats over a fixed grid with no way to adjust them. The candidate grid is also now configurable for sPLS-DA
- **Documentation**: New "Prediction Distance Metrics" section in `docs/help/lda/details.md` and two FAQ entries covering which rule to use and what disagreement between rules means; expanded "Choosing keepX" in `docs/help/pca/details.md` explaining the tuning criterion and how to read the new curve; new PCA FAQ entry "How do I justify my keepX in a paper?"

### Changed

- **sPCA keepX tuning reports progress and failures**: The tuning button previously ran synchronously with no progress indication and swallowed failures into a log line — the user pressed it and nothing visibly happened, including when the run legitimately failed (e.g. fewer than 3 repeats, which the logic layer explicitly guards against). It now shows a progress bar and start/success/error notifications, matching the LDA module's existing behaviour. Component-count diagnostics in the LDA module likewise gained a progress bar and error notification
- **`run_pca_tune_keepx()` and `run_plsda_tune_keepx()` return richer results**: both now return `$keep_x` alongside the CV settings actually used (and, for sPCA, `$cor_comp`), so the tuning can be reported rather than just applied. Callers and tests updated
- **`tune.splsda()` distance and measure are now explicit**: previously left to mixOmics' defaults and therefore invisible; now passed as `dist = "max.dist"`, `measure = "BER"` with the same effective behaviour, making the choice visible, testable and configurable

### Fixed

- **`perf()` distance argument made explicit**: `run_plsda_perf()` relied on `dist` defaulting to `"all"` while depending on all three rules being present. Now stated explicitly, and the comparison degrades gracefully when mixOmics drops `mahalanobis.dist` on a singular covariance matrix rather than assuming three columns exist

## [2026.14] - 2026-08-19

### Added

- **Prediction diagnostics**: The Prediction module now reports quantitative confidence/distance metrics alongside predicted class and posterior probability, so a group assignment can be reported as statistically defensible rather than relying on overlay-plot position alone. PCA/sPCA/IPCA bundles get Hotelling's T² and Q-residual (SPE), with a threshold-flagged "Prediction Diagnostics" table — this catches the sPCA/IPCA-specific case where a sample looks normal on the retained components but is structurally unlike training data. LDA/QDA/MDA/PLS-DA/sPLS-DA bundles get Mahalanobis distance and typicality probability (original measurement space for LDA/QDA/MDA, component-score space for PLS-DA/sPLS-DA) to catch specimens that get a confident posterior despite belonging to no represented group. Cluster bundles get a distance-ratio confidence proxy (own-centroid vs. second-nearest). Diagnostics-flagged rows are highlighted in the results table
- **Training Model Quality panel**: New panel in the Prediction module surfacing the training model's own classification performance (confusion matrix, per-class precision/recall/F1, overall accuracy) with its evaluation source labelled (cross-validated / held-out test / resubstitution), so a prediction's trustworthiness can be judged alongside its confidence. Previously this was computed during LDA/QDA/MDA/PLS-DA/sPLS-DA training but discarded at RDS-bundle export time
- **Prediction Excel export**: Extended with a Diagnostics sheet and Training Confusion Matrix / Training Per-Class Metrics sheets, matching the existing multi-sheet precedent from LDA export
- **Documentation**: `docs/help/prediction/details.md` gained interpretation sections for T²/Q, Mahalanobis/typicality, and the distance-ratio proxy; `docs/help/prediction/faq.md` gained two conceptual FAQ entries plus the standard "What should I report in a paper or thesis?" entry, and corrected stale claims (PCA prediction described `predict.prcomp()`, though PCA/sPCA/IPCA have no `predict()` method and use manual projection like every other type; the Requirements table listed only 5 of the 9 supported analysis types)
...
- **Prediction diagnostics table was unlabeled and unreadable**: The new T²/Q and Mahalanobis/typicality table had no `Sample` column, so rows could not be matched to specimens, and flagged rows (`T2_flag`/`Q_flag`/low typicality/high distance-ratio) had no visual emphasis beyond a literal `true`/`false` cell. Sample labels now match the Prediction Results panel, and flagged rows are highlighted
- **Training confusion matrix had an unlabeled row axis**: The true-class row header was rendered as an anonymous DT row index, indistinguishable from a table row number — now labelled `True \ Predicted`, matching the existing Excel export
