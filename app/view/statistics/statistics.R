...
box::use(
  ...
  app/view/statistics/ranking_table,
)
...
server <- function(id, input_data, data_version,
  ...
  shiny$moduleServer(id, function(input, output, session) {
    ...
    shiny$observeEvent(input$compute_button, {
      ...
      # Screening mode (plots_available() == FALSE): no plots exist by
      # design, so the plot-availability check below is skipped entirely.
      plots_enabled <- if (!is.null(plots_available)) {
        isTRUE(plots_available())
      } else {
        TRUE
      }
      ...
      if (status == "done") {
        ...
        measurement_cards <- lapply(
          ...
          function(m) {
            ...
            plot_ui <- if (isTRUE(results$no_plots_mode)) {
              # Screening mode: plots were never generated by design,
              # so omit the section entirely rather than showing the
              # "no plot" fallback (which is meant to flag the
              # unexpected case) once per measurement.
              NULL
            } else if (has_plot) {
              shiny$tags$div(
                class = paste(
