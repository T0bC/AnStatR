...
function initializeWindowSize(targetId, windowInputId) {
    ...
    //
    // Keep in sync with app/static/js/plot_resize.js, which holds the copy
    // of this function that main.R actually loads (webpack tree-shakes
    // initializeWindowSize out of app.min.js, since nothing in this bundle
    // calls it).
    //
    ...
    //
    // Instead the measurement is deferred to idle time, by which point the
    // browser has laid out the new content on its own schedule and the read
    // is free. It is also coalesced, so one burst of mutations produces one
    // measurement rather than one per batch.
    ...
    // tab woke all of them. targetId is a uiOutput container, so Shiny
    // replaces its children and the element itself persists -- safe to
    // observe once and keep.
    var attached = false;
    var attempts = 0;
...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // This block exists verbatim in app/static/js/plot_resize.js too, and
        // both files are loaded: main.R adds plot_resize.js explicitly, while
        // rhino's with_head_tags() auto-injects app.min.js (built from this
        // file). Without this claim the observer would attach twice and every
        // DOM insertion would be swept twice. First one in wins; the other
        // becomes a no-op.
...
(function () {
    ...
    //
    // Coalesced to one sweep per frame for the same reason as the girafe
    // observer above: running querySelectorAll() per added node meant
    // walking the whole inserted subtree once per node, which was expensive
    // when a plot inserted tens of thousands of nodes. injectHandle() is
    // already idempotent (it returns early if the handle exists), so a
    // document-wide sweep is safe to repeat.
