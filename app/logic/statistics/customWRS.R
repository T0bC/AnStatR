...
smmcrit01 <- function(nuhat, C) {
  ...
  if (C >= 2) {
    ...
    m1 <- matrix(0, 20, 27)
    m1[1, ] <- c(
      12.73, 14.44, 15.65, 16.59, 17.35, 17.99,
...
lincon1 <- function(x, con = 0, tr = 0.2, alpha = 0.05, pr = TRUE, crit = NA,
  ...
  if (sum(con ^ 2) == 0) {
    ...
    dimnames(psihat) <- list(NULL, c(
      "Group", "Group", "psihat",
      "ci.lower", "ci.upper", "p.value"
    ))
    test <- matrix(NA, CC, 6)
    dimnames(test) <- list(NULL, c(
      "Group", "Group", "test",
      "crit", "se", "df"
    ))
    jcom <- 0
...
mcp2atm <- function(formula, data, tr = 0.2, ...) {
  ...
  dataMelt <- melt(data,
    id = c("row", colnames(mf)[2], colnames(mf)[3]),
    measured = mf[, 1]
  )
  dataWide <- cast(dataMelt, as.formula(paste(
    colnames(dataMelt)[1],
    "~", colnames(mf)[2], "+", colnames(mf)[3]
  )), fun.aggregate = mean)
  dataWide$row <- NULL
  ...
  dnamesAB <- apply(expand.grid(dnamesA, dnamesB), 1, function(ss) {
    paste(ss[1],
      ss[2],
      sep = ":"
    )
  ...
  result <- list(
    effects = effects, contrasts = contrasts,
    call = cl
  )
  class(result) <- "mcp"
  result
...
mcp2atm_TM <- function(formula, data, tr = 0.2, ...) {
  ...
  conB_X <- conB
  dnamesAB_X <- apply(expand.grid(dnamesA_X, dnamesB_X), 1, function(ss) {
    paste(ss[1],
      ss[2],
      sep = ":"
    )
