...
- **Results do not update automatically.** Changing a filter after computing leaves the existing results on screen and shows an amber **Data selection changed** notice above the **Compute Discriminant Analysis** button. This is deliberate: results are kept so you can compare, and are never silently relabelled. Press **Compute Discriminant Analysis** again to recompute.
...
Both are reported using the `max.dist` classification rule (assign to the class with maximum predicted score), which is the rule `mixOmics::predict()` applies and therefore the one the accuracy and confusion matrix elsewhere in this module are based on. The same run also reports all three prediction distances side by side — see **Prediction Distance Metrics** below. Look for the component count where error stops decreasing meaningfully (an "elbow") — adding components beyond that point usually adds noise, not signal, to the model.
...
**Choosing folds and repeats**: the mixOmics documentation advises that each fold should contain **at least 5–6 samples** (so with 20 specimens, do not use more than 4 folds; below 3 samples per fold the run fails outright), and that a **final reported result should use 50–100 repeats**. The app's defaults (5 folds × 10 repeats) are set for responsiveness, not for publication: they give a quick provisional answer. The sidebar warns when your settings fall below either guideline. Raise the repeat count before quoting these numbers in a manuscript — the estimate is averaged over random partitions, and too few repeats means the figure moves depending on the luck of the split. Fixing a random seed is *not* a substitute for more repeats; it makes an unstable number reproducible without making it reliable.

**Suggested Component Count (mixOmics)**: alongside the error curve, the panel reports mixOmics' own recommendation, derived from one-sided t-tests comparing each component against the previous one, broken down per error measure and per distance rule. This is a second opinion on the dashed line in the plot, which applies a simpler parsimony rule (smallest count within 1 pp of the best BER). When every cell of that table agrees, the component count is well determined; when they disagree, prefer the BER row for unbalanced group sizes and favour the smaller count. Neither recommendation is applied automatically.

**Error Rate per Group**: a per-group breakdown of the cross-validated error, at the fitted component count, under each distance rule. This is the panel's most diagnostic output and the reason not to read the overall figures alone — an acceptable overall BER can conceal one group being classified at or below chance while the others carry the average. If one group stands out, check whether it is small, internally heterogeneous, or genuinely overlaps another in component space. Note these are *cross-validated*, unlike the per-class metrics in the Confusion Matrix panel, which are measured on the same specimens the model was fitted to and are therefore optimistic.
...
<details>
<summary><strong>Prediction Distance Metrics</strong></summary>

Once a PLS-DA/sPLS-DA model has produced component scores for a sample, that sample still has to be *assigned* to a group. mixOmics offers three rules for doing so, and `perf()` computes the cross-validated error under all three in the same run. The **Prediction Distance Comparison** table in the Component Diagnostics panel shows them side by side.

| Rule | How a sample is assigned | Best suited to |
|------|--------------------------|----------------|
| **`max.dist`** (maximum distance) | The predicted response $\hat{y}$ is computed for each class and the sample goes to the class with the largest value. This is a regression rule, not a geometric one — it uses the model's fitted response directly. | The default, and the rule mixOmics uses in `predict()`. Generally the best performer for discriminant tasks, and robust when classes are well separated. |
| **`centroids.dist`** (centroid distance) | The Euclidean distance from the sample to each class centroid in component space; nearest centroid wins. | Groups that are roughly spherical and of similar spread. Often gives smoother, more intuitive decision regions. |
| **`mahalanobis.dist`** (Mahalanobis distance) | As above, but distances are measured under the pooled component covariance, so correlated or elongated component spread is accounted for. | Groups whose scatter is elongated or whose components are correlated. Requires an invertible covariance matrix — mixOmics drops this rule when the covariance is singular. |

**Why the app reports `max.dist`**: the accuracy figure, the confusion matrix, the posterior probabilities and the saved `.rds` prediction bundle all use `max.dist`, because that is what `mixOmics::predict()` returns and consistency between the reported number and the exported model matters more than squeezing out a marginally lower error. The comparison table does not change any of those numbers — it is evidence, not a setting.

**How to read the comparison**: what matters is the *spread* between the rules, which the app states in percentage points beneath the table.

- **Spread under ~2 pp** — the rules agree. The choice of distance is not a meaningful decision for your data, and you need not mention it beyond naming the one you used.
- **Spread of ~2–5 pp** — a real but modest difference. Worth noting in a methods section; rarely changes a conclusion.
- **Spread above ~5 pp** — the rules disagree substantially. This is diagnostic information in itself: it usually means the groups are *not* cleanly separated in component space, so the headline accuracy is one of several defensible numbers rather than a single fact about the data. Report which rule you used, and treat the between-rule difference as part of the uncertainty rather than picking the flattering one after the fact.

Choosing the rule that happens to give the lowest error, after seeing the table, is a form of selection on the test statistic — it biases the reported accuracy optimistically. The honest use of this table is to check *whether* the choice matters, not to shop for a better number.

**Where the rule is user-selectable**: the **Boundary rule** control (Plotting Controls → shown when decision boundaries are enabled) changes only how the decision-boundary *background* of the scores plot is drawn. It is deliberately cosmetic and does not affect any reported statistic. Note that outside the Comp1-vs-Comp2 view, `max.dist` falls back to centroid distance there, since its regression rule only applies to the leading component pair.
